# Tailored Resume for Connecteam: Customer Support Representative

```text
ISAIAH WASHINGTON
Cincinnati, OH | Open to Austin, TX and Remote
513.544.9022 | isaiahwashington48@gmail.com
linkedin.com/in/isaiah-washington48 | github.com/Izayauh

SUMMARY
Technical support professional with 4+ years of remote, client-facing experience resolving infrastructure and software issues. Employs a can-do attitude to deliver fast service and maintain high performance levels, completing 1,500+ hours of direct customer interaction across 150+ managed properties. Tech-savvy problem solver who builds independent Python and local ML projects in fast-paced environments.

SKILLS
Support & Operations: Client-facing communication, providing live feedback, fast-paced ticket lifecycle management, remote troubleshooting, educating clients on best practices.
Tools & Platforms: Windows OS, Git, GitHub, Microsoft Office.
Programming: Python, HTML/CSS/JavaScript, SQL (Coursework).

EXPERIENCE
Network Support Technician | Nexus WiFi
West Chester, OH (100% Remote) | Oct 2021 – Present
• Serve as the main point of contact for external customers across 150+ managed properties, ensuring client success and satisfaction through various communication channels.
• Triage connectivity outages and conduct fault isolation across access points, switches, gateways, and modems.
• Execute diagnostic workflows using internal ticketing platforms to resolve inbound support requests and prioritize effectively to ensure clients get the best service.
• Provide positive, high-energy written and verbal communication to non-technical users to educate them on diagnostic steps and resolve day-to-day problems.

Key Accounts Team Assistant | The Cincinnati Insurance Company
Fairfield, OH | Feb 2023 – Aug 2023
• Managed data input for key insured account policies, providing feedback and sharing ideas with underwriting teams to ensure accurate policy data input.

Administrative Assistant | Midway Auto Group
Middletown, OH | Jan 2019 – Dec 2023
• Built and maintained a spreadsheet-based inventory tracking system covering cost, deliveries, and vehicle usage.

PROJECTS
Impulse — Privacy-First Dictation App (Beta)
github.com/Izayauh/whisper | impulse-eight-lake.vercel.app
• Built a Windows speech-to-text dictation application using Python, OpenAI Whisper, and NVIDIA CUDA for local inference.
• Created a Windows installer, UI dashboard, and hotkey system.

Live Pilot — AI DAW Assistant (Functional Prototype)
github.com/Izayauh/LivePilot
• Built a voice-controlled DAW assistant using Python, the Google Gemini API, and the OSC protocol.

EDUCATION
Miami University
Business Management Technology coursework
Completed 67 credit hours toward Associate of Applied Business
Attended through Spring 2024

Google IT Support Certification — Coursera, November 2022
```
